...
public:
     ...
     void startWriting()
     {
          ...
          boost::asio::async_write(socket_, boost::asio::buffer(final), boost::bind(&tcp_connection::writeHandler, shared_from_this(), boost::asio::placeholders::error));
     }
     ...
     void readMessages()
     {
          boost::asio::async_read_until(socket_, buffer_, "\n", boost::bind(&tcp_connection::handleRead, shared_from_this(), boost::asio::placeholders::error, boost::asio::placeholders::bytes_transferred
...
{
public:
     tcp_client(boost::asio::io_context &io_context, const std::string &username)
         : io_context_{io_context}, username_{username}, resolver_{io_context},
     ...
     void resolverHandler(const boost::system::error_code &error, tcp::resolver::results_type results)
     {
          if (!error)
          {
               ...
               boost::asio::async_connect(new_connection->socket(), results, boost::bind(&tcp_client::handleConnection, this, new_connection, boost::asio::placeholders::error));
          }
          else
          {
               ////
